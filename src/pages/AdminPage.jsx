...
/* Fonction non incluse dans le plan : renvoie vers la page des plans */
function PlanLock({ children }) {
  return (
...
export default function AdminPage() {
  ...
  // Ce que le plan de la librairie autorise (calculé par l'API)
  const caps = currentUser?.plan_capabilities || {};
  const isLocked = (key) => Array.isArray(stats?.locked) && stats.locked.includes(key);
  ...
  const load = useCallback(() => {
    ...
    api.get('/orders', { params: { scope: 'shop' } }).then(r=>setOrders(r.data)).catch(()=>{});
  ...
  const downloadExport = async (type) => {
    try {
      ...
      let msg = "Impossible de générer l'export";
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        {currentUser && (
          <div className="flex items-center gap-3 bg-white border border-[#e8e0d4] rounded-xl px-4 py-2.5 shadow-sm">
            ...
            <span className="ml-2 text-xs bg-[#c9933a]/10 text-[#c9933a] font-bold px-2 py-0.5 rounded-full border border-[#c9933a]/20">
              {currentUser.is_super_admin ? '👑 Super Admin' : currentUser.is_shop_member ? '🤝 Collaborateur' : '🏪 Libraire'}
            </span>
            {!currentUser.is_super_admin && caps.plan_name && (
              <span className="ml-2 text-xs font-semibold text-gray-500">
                Plan {caps.plan_name}
                {caps.max_products != null && ` · ${(stats?.nbBooks ?? 0) + (stats?.nbFournitures ?? 0)}/${caps.max_products} produits`}
                {caps.commission_rate != null && ` · commission ${String(caps.commission_rate).replace('.', ',')} %`}
              </span>
            )}
          </div>
        )}
        ...
        <div>
          {!stats ? (
            ...
            <>
              ...
              {isLocked('topSelling') && (
                <div className="mb-6"><PlanLock>Votre plan inclut les statistiques de base (ventes et stock). Top produits, tendances et produits consultés sont inclus à partir du plan Professionnel.</PlanLock></div>
              )}
              {stats.caLast30Days && (
                <div className="grid md:grid-cols-2 gap-6 mb-6">
                  <div className="border rounded-xl p-5">
                    <h3 className="font-semibold mb-1">📈 Chiffre d'affaires sur 30 jours</h3>
                    <Sparkline data={stats.caLast30Days}/>
                    {stats.caLast30Days.length>0 && <p className="text-sm text-gray-500 mt-2">Total : <strong>{formatCFA(stats.caLast30Days.reduce((t,d)=>t+d.ca,0))}</strong></p>}
                  </div>
                  <div className="border rounded-xl p-5">
                    <h3 className="font-semibold mb-3">🗂️ Ventes par catégorie</h3>
                    {!stats.salesByCategory?.length ? <p className="text-gray-400 text-sm">Pas encore de données</p> : (
                      <div className="space-y-2">{stats.salesByCategory.map(c=>(
                        <div key={c.categorie} className="flex justify-between text-sm"><span>{c.categorie} <span className="text-gray-400">({c.quantite})</span></span><strong>{formatCFA(c.ca)}</strong></div>
                      ))}</div>
                    )}
                  </div>
                </div>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div className="border rounded-xl p-5">
                  ...
                  {isLocked('caLastDays')
                    ? <PlanLock>La tendance du chiffre d'affaires est incluse à partir du plan Professionnel.</PlanLock>
      ...
      {tab==='promos' && (
        <div>
          {!caps.promotions && !currentUser?.is_super_admin && (
            <div className="mb-6"><PlanLock>La gestion des promotions est incluse à partir du plan Professionnel. Les promotions existantes restent visibles mais ne peuvent plus être créées ni réactivées.</PlanLock></div>
          )}
          {(caps.promotions || currentUser?.is_super_admin) && <form onSubmit={handlePromoSubmit} className="bg-white border rounded-xl p-6 mb-8 shadow-sm">
          ...
          {!caps.coupons && !currentUser?.is_super_admin && (
            <div className="mt-10"><PlanLock>Les codes promo de la librairie sont inclus à partir du plan Professionnel.</PlanLock></div>
          )}
          {(caps.coupons || currentUser?.is_super_admin) && (
            <ShopCouponsPanel allowed isSuperAdmin={!!currentUser?.is_super_admin} />
          )}
        </div>
      )}
