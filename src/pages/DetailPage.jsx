...
function InfoRow({ label, value, mono=false }) {
  ...
  return (
    <div className="flex gap-3 py-2.5 border-b border-[#f0ece4] last:border-0">
      <span className="w-32 flex-shrink-0 text-xs font-semibold text-gray-400 uppercase tracking-wide pt-0.5">{label}</span>
...
=======
      <div className="grid grid-cols-2 gap-3 text-sm">
        <div><p className="text-xs text-gray-400">Remise</p><p className="font-bold text-[#d44040]">{promo.type==='percentage'?`-${promo.value}%`:`-${formatCFA(promo.value)}`}</p></div>
        <div><p className="text-xs text-gray-400">Économie</p><p className="font-bold text-[#2d7a4f]">{formatCFA(savings)}</p></div>
        {promo.start_at&&<div><p className="text-xs text-gray-400">Du</p><p>{new Date(promo.start_at).toLocaleDateString('fr-FR')}</p></div>}
        {promo.end_at&&<div><p className="text-xs text-gray-400">Au</p><p>{new Date(promo.end_at).toLocaleDateString('fr-FR')}</p></div>}
      </div>
      {timeLeft&&timeLeft!=='Terminée'&&(
        <div className="mt-3 flex items-center gap-2 bg-amber-100 rounded-xl px-3 py-2">
          <span>⏱</span><span className="text-sm font-semibold text-amber-800">Expire dans <strong>{timeLeft}</strong></span>
>>>>>>> 294c6dc (Initial commit)
        </div>
      )}
    </div>
...
=======
        addRecentlyViewed({ id:res.data.id, type:'book', nom:res.data.titre, image:res.data.image_url||res.data.image||'📖', prix:res.data.final_price??res.data.prix });
>>>>>>> 294c6dc (Initial commit)
      })
...
=======
    ? (book.comments.reduce((s,c)=>s+c.note,0)/book.comments.length).toFixed(1) : null;

  const handleAddToCart = async () => {
    if (!user) { showToast('Connectez-vous pour ajouter au panier','error'); navigate('/login',{state:{from:{pathname:`/book/${id}`}}}); return; }
    setAddingCart(true);
    try { await addToCart(book.id, qty); showToast(`✅ "${book.titre}" ajouté au panier (×${qty})`,'success'); }
    catch (err) { showToast(err.response?.data?.message||'Erreur','error'); }
    finally { setAddingCart(false); }
  };

  const handleCommentSubmit = async (e) => {
    e.preventDefault(); if (!comment.contenu.trim()) return; setSubmitting(true);
    try {
      const res = await api.post(`/books/${book.id}/comment`, comment);
      setBook(prev=>({...prev, comments:[...(prev.comments||[]),res.data]}));
      setComment({contenu:'',note:5}); showToast('Commentaire publié !','success');
    } catch(err) { showToast(err.response?.data?.message||'Vous avez peut-être déjà commenté','error'); }
    finally { setSubmitting(false); }
  };
...
=======
  const nbPhotos      = (book.images?.length ?? 0) + (book.image_url ? 1 : 0);
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <nav className="flex items-center gap-2 text-xs text-gray-400 mb-8 flex-wrap">
        ...
        <Link to="/catalog" className="hover:text-[#c9933a] transition">Catalogue</Link><span>›</span>
        <Link to={`/catalog?rayon=${encodeURIComponent(book.rayon)}`} className="hover:text-[#c9933a] transition">{book.rayon}</Link><span>›</span>
        <span className="text-gray-600 truncate max-w-[200px]">{book.titre}</span>
      </nav>
      ...
      <div className="flex flex-col md:flex-row gap-10">
        ...
        <div className="md:w-5/12 flex-shrink-0">
          <ImageGallery
            ...
            alt={book.titre}
            fallback={book.image || '📖'}
            hasBadge={hasPromo ? `-${discountPercent(originalPrice,finalPrice)}%` : null}
          />

          {nbPhotos > 0 && (
            <p className="text-center text-xs text-gray-400 mt-2">
              {nbPhotos} photo{nbPhotos>1?'s':''} disponible{nbPhotos>1?'s':''} · Cliquez pour agrandir
            </p>
          )}
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-lg text-gray-600 mb-1 font-medium">{book.auteur}</p>
          <p className="text-gray-400 text-sm mb-4">{book.editeur}{book.annee?` · ${book.annee}`:''}</p>

          {/* Note moyenne */}
          {avgNote && (
            <div className="flex items-center gap-3 mb-5 bg-amber-50 border border-amber-100 rounded-xl px-4 py-2.5 inline-flex w-fit">
>>>>>>> 294c6dc (Initial commit)
...
=======
          {hasPromo && promo && <PromoBlock promo={promo} originalPrice={originalPrice} finalPrice={finalPrice} />}
...
=======
            {book.quantite > 0
              ? <span className="text-sm bg-green-50 text-[#2d7a4f] border border-green-200 px-3 py-1.5 rounded-xl font-medium">✓ En stock ({book.quantite})</span>
          ...
          {book.quantite > 0 && (
            <div className="flex items-center gap-3 mb-8 flex-wrap">
              <div className="flex items-center border-2 border-[#e8e0d4] rounded-xl overflow-hidden">
                ...
                <span className="px-5 py-2.5 font-bold text-[#0f1923] min-w-[44px] text-center">{qty}</span>
                <button onClick={() => setQty(q=>Math.min(book.quantite,q+1))} className="px-4 py-2.5 bg-[#faf7f2] hover:bg-[#ede5d4] text-xl font-bold transition text-[#0f1923]">+</button>
              </div>
              <button onClick={handleAddToCart} disabled={addingCart}
                className={`flex-1 min-w-[180px] py-3 rounded-xl font-bold transition-all active:scale-[0.98] ${addingCart?'bg-[#2d7a4f] text-white':'bg-[#0f1923] text-white hover:bg-[#c9933a]'} disabled:opacity-70`}>
                {addingCart ? '✓ Ajouté au panier !' : '🛒 Ajouter au panier'}
              </button>
              {user && (
                <button onClick={() => toggleWish(book.id,'book')}
          ...
          <div className="border border-[#e8e0d4] rounded-2xl overflow-hidden">
            <div className="flex border-b border-[#e8e0d4]">
              {[['description','📄 Description'],['details','📋 Fiche technique'],['livraison','🚚 Livraison']].map(([key,label])=>(
            ...
            <div className="p-5 bg-white">
              ...
              {tab==='details' && (
                <div className="divide-y divide-[#f0ece4]">
                  <InfoRow label="Titre"     value={book.titre} />
                  <InfoRow label="Auteur"    value={book.auteur} />
                  <InfoRow label="Éditeur"   value={book.editeur} />
                  <InfoRow label="ISBN"      value={book.isbn} mono />
                  <InfoRow label="Année"     value={book.annee} />
                  <InfoRow label="Rayon"     value={book.rayon} />
                  <InfoRow label="Stock"     value={`${book.quantite} exemplaire${book.quantite>1?'s':''}`} />
                  <InfoRow label="Réf. interne" value={`LIV-${String(book.id).padStart(5,'0')}`} mono />
                  {hasPromo && <InfoRow label="Promotion" value={`-${discountPercent(originalPrice,finalPrice)}% → ${formatCFA(finalPrice)}`} />}
                </div>
              )}
              ...
              {tab==='livraison' && (
                <div className="space-y-3 text-sm text-gray-600">
                  ...
                  <div className="flex gap-3 items-start"><span className="text-2xl">🔄</span><div><p className="font-semibold text-[#0f1923]">Retour & échange</p><p className="text-xs text-gray-400">7 jours après réception, si non ouvert</p></div></div>
                </div>
              )}
            </div>
>>>>>>> 294c6dc (Initial commit)
...
=======
      <div className="mt-12 border-t border-[#e8e0d4] pt-10 space-y-10">
        <RecommendedProducts productId={book.id} productType="book" onAddToCart={(id,type)=>addToCart(id,1,type)} />
>>>>>>> 294c6dc (Initial commit)
...
=======
          Avis des lecteurs {book.comments?.length>0&&<span className="text-gray-400 text-lg font-normal ml-2">({book.comments.length})</span>}
        </h2>

        {user ? (
          <form onSubmit={handleCommentSubmit} className="bg-white border border-[#e8e0d4] rounded-2xl p-6 mb-10 shadow-sm">
            <h3 className="font-bold text-[#0f1923] mb-4">Laissez votre avis</h3>
            <div className="mb-4"><label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Votre note</label><Stars note={comment.note} interactive onChange={n=>setComment(c=>({...c,note:n}))} /></div>
            <textarea value={comment.contenu} onChange={e=>setComment(c=>({...c,contenu:e.target.value}))}
              placeholder="Partagez votre avis sur ce livre…"
              className="w-full px-4 py-3 border border-[#e8e0d4] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#c9933a]/30 focus:border-[#c9933a] text-sm transition mb-4" rows={3} required/>
            <button type="submit" disabled={submitting||!comment.contenu.trim()}
              className="bg-[#0f1923] text-white px-6 py-2.5 rounded-xl font-semibold hover:bg-[#c9933a] transition disabled:opacity-50">
              {submitting?'⏳ Publication…':'✍️ Publier mon avis'}
>>>>>>> 294c6dc (Initial commit)
...
=======
            <p className="text-sm text-amber-800"><Link to="/login" state={{from:{pathname:`/book/${id}`}}} className="font-bold underline">Connectez-vous</Link> pour laisser un avis.</p>
>>>>>>> 294c6dc (Initial commit)
          </div>
        )}
