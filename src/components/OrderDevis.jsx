...
function qrUrl(data, size = 130) {
  return `https://api.qrserver.com/v1/create-qr-code/?size=${size}x${size}&data=${encodeURIComponent(
    data
  )}&bgcolor=ffffff&color=1e3a5f&qzone=1`;
}
...
function SignaturePad({ onSave, savedSignature }) {
  ...
  useEffect(() => {
    setEmpty(!savedSignature);
    setSaved(!!savedSignature);
    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="flex gap-2">
        <button
          type="button"
          onClick={clear}
          className="flex-1 text-xs bg-gray-100 text-gray-600 px-3 py-1.5 rounded-lg hover:bg-gray-200 transition"
        >
          Effacer
        </button>
        ...
        <button
          ...
          onClick={save}
          disabled={empty}
          className={`flex-1 text-xs px-3 py-1.5 rounded-lg transition font-semibold ${
            saved
...
export default function OrderDevis({
  order,
  isAdmin = false,
  autoExport = false,
}) {
  ...
  const numero = order?.numero_commande || (order?.id ? `CMD-${order.id}` : 'DEVIS');
  ...
  const originalTotal = items.reduce(
    (sum, item) =>
      sum +
      parseFloat(item.book?.prix ?? item.prix_unitaire ?? 0) *
  ...
  useEffect(() => {
    if (!order || !autoExport || autoFired.current) return;
  ...
  }, [autoExport, exportPDF, order]);

  if (!order) return null;

  const deliveryMode = order.delivery_mode;
  const deliveryFee = parseFloat(order.frais_livraison ?? 0);
  const couponDiscount = parseFloat(order.coupon_remise ?? 0);

  const storeAddress = LIBRAIRE_ADR;
  const storePhone = LIBRAIRE_TEL;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-3 items-center no-print">
        ...
        <button
          type="button"
          onClick={() => setShowSigs((value) => !value)}
      ...
      <div
        ref={devisRef}
        className="bg-white rounded-2xl overflow-hidden shadow-xl border border-gray-100 max-w-2xl mx-auto"
      ...
      >
        ...
        <div className="grid grid-cols-2 border-b">
          <div className="px-7 py-5 border-r bg-slate-50">
            ...
            <p className="text-gray-600 text-sm mt-1 leading-snug">
              📍 {order.adresse ?? 'Adresse non renseignée'}
            </p>
            ...
            <div className="mt-2 space-y-1">
              ...
              {clientEmail && (
                <p className="text-xs text-gray-500">✉️ {clientEmail}</p>
              )}
            </div>
          </div>
          ...
          <div className="px-7 py-2.5 bg-emerald-50 flex flex-wrap items-center gap-4">
            ...
            {paymentMethod && (
              <span className="text-sm font-semibold text-gray-600">
                💳 Paiement :{' '}
                <span className="text-[#1e3a5f]">{paymentMethod}</span>
              </span>
            )}
          </div>
          ...
          >
            ...
            {deliveryMode !== 'pickup' && deliveryFee > 0 && (
              <span className="text-xs font-bold text-amber-700 bg-amber-100 px-2.5 py-1 rounded-full flex-shrink-0">
                +{new Intl.NumberFormat('fr-FR').format(deliveryFee)} FCFA
              </span>
            )}
          </div>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {items.length === 0 ? (
                ...
                items.map((item) => {
                  const puFinal = parseFloat(
                    item.prix_unitaire ?? item.book?.prix ?? 0
                  );
                  ...
                  return (
                    <tr key={item.id}>
                          ...
                          <div>
                            ...
                            {hasPromo && (
                              <span className="text-[10px] font-bold text-red-500 bg-red-50 px-1.5 rounded">
                                PROMO -{discountPercent(puOrig, puFinal)}%
                              </span>
                            )}
                          </div>
                        </div>
                      ...
                      <td className="py-3 text-right font-bold text-[#1e3a5f]">
                        {formatCFA(puFinal * Number(item.quantite ?? 0))}
                      </td>
                    </tr>
                  );
                })
              ...
              <>
                <div className="flex justify-between text-sm text-gray-500">
                  ...
                  <span className="line-through">
                    {formatCFA(originalTotal)}
                  </span>
                </div>

                <div className="flex justify-between text-sm text-emerald-700 font-semibold">
                  <span>🎉 Remises promotions</span>
                  <span>-{formatCFA(savings)}</span>
                </div>
              </>
            )}
        ...
        <div className="border-t mx-7 pt-5 pb-5 flex flex-col sm:flex-row gap-6 items-start">
          ...
          <div className="flex-1 grid grid-cols-2 gap-4">
            {['client', 'lib'].map((who) => {
              ...
              const label =
                who === 'client' ? 'Signature client' : 'Cachet librairie';
              const name =
                who === 'client' ? order.client_nom : LIBRAIRE_NOM;
        ...
        <div className="bg-[#1e3a5f]/5 border-t px-7 py-3 text-[10px] text-gray-400 text-center">
          ...
          <br />
          Ce bon de commande est généré automatiquement et fait foi de la
          transaction.
        </div>
      </div>
      ...
      {showSigs && (
        <div className="grid sm:grid-cols-2 gap-4 no-print">
          ...
          {isAdmin && (
            <div className="bg-white border rounded-xl p-4 shadow-sm">
              <p className="font-semibold text-sm text-gray-700 mb-3">
                🏪 Cachet de la librairie
              </p>
              <SignaturePad
                onSave={setLibSig}
                savedSignature={libSig}
              />
            </div>
          )}
        </div>
